...
impl PasswdEntry {
    ...
    pub fn parse(s: &str) -> Option<PasswdEntry> {
        ...
        Some(PasswdEntry {
            username: match entries.next() { None => return None, Some(s) => s.to_string() },
            passwd: match entries.next() { None => return None, Some(s) => s.to_string() },
            ...
            gecos: match entries.next() { None => return None, Some(s) => s.to_string() },
            home_dir: match entries.next() { None => return None, Some(s) => s.to_string() },
            shell: match entries.next() { None => return None, Some(s) => s.to_string() },
        })
    }
}
...
impl GroupEntry {
    ...
    pub fn parse(s: &str) -> Option<GroupEntry> {
        ...
        Some(GroupEntry {
            name: match entries.next() { None => return None, Some(s) => s.to_string()},
            passwd: match entries.next() { None => return None, Some(s) => s.to_string() },
            gid: match entries.next().and_then(|s| s.parse().ok()) { None => return None, Some(s) => s },
            users: match entries.next() { None => return None, Some(s) => s.split(',').map(|p| p.to_string()).collect() },
        })
    }
}
...
impl PasswdReader {
    ///Creates a new PasswdReader with a 
    ...
    pub fn new(cache_time: Duration) -> Self {
        ...
        Self { file: None, cache_time, last_check, passwd: vec![] }
    }
    ...
    pub fn new_at(file: &str, cache_time: Duration) -> Self {
        let last_check = Instant::now() - (cache_time);
        Self { file: Some(file.to_string()), cache_time, last_check, passwd: vec![] }
    }
    ...
    async fn refresh_if_needed(&mut self) -> Result<(), std::io::Error>{
        ...
        let contents = tokio::fs::read_to_string(self.file.as_ref().unwrap_or(&"/etc/passwd".to_string())).await?;
    ...
    pub async fn get_by_username(&mut self, username: &str) -> Result<Option<PasswdEntry>, std::io::Error> {
        self.refresh_if_needed().await?;
        Ok(self.passwd.iter().find(|e| e.username == username).map(|e| e.to_owned()))
    ...
    pub async fn get_by_uid(&mut self, uid: u32) -> Result<Option<PasswdEntry>, std::io::Error> {
        self.refresh_if_needed().await?;
        Ok(self.passwd.iter().find(|e| e.uid == uid).map(|e| e.to_owned()))

    }
    
    ///Look up a username by uid
    pub async fn get_username_by_uid(&mut self, uid: u32) -> Result<Option<String>, std::io::Error> {
        self.refresh_if_needed().await?;
        Ok(self.passwd.iter().find(|e| e.uid == uid).map(|e| e.username.to_owned()))
    }

    ///Look up a user ID by username
    pub async fn get_uid_by_username(&mut self, username: &str) -> Result<Option<u32>, std::io::Error> {
        self.refresh_if_needed().await?;
        Ok(self.passwd.iter().find(|e| e.username == username).map(|e| e.uid))
...
impl GroupReader {
    ///Creates a new GroupReader with a 
    ///specified cache_time in seconds. 
    ///
    ///Use cache_time with a duration of 0 to disable caching.
    pub fn new(cache_time: Duration) -> Self {
        let last_check = Instant::now() - (cache_time);
        Self { file: None, cache_time, last_check, groups: vec![] }
    }
    ...
    pub fn new_at(file: &str, cache_time: Duration) -> Self {
        let last_check = Instant::now() - (cache_time);
        Self { file: Some(file.to_string()), cache_time, last_check, groups: vec![] }
    }
    ...
    async fn refresh_if_needed(&mut self) -> Result<(), std::io::Error>{
        ...
        let contents = tokio::fs::read_to_string(self.file.as_ref().unwrap_or(&"/etc/group".to_string())).await?;
    ...
    pub async fn get_by_name(&mut self, name: &str) -> Result<Option<GroupEntry>, std::io::Error> {
        self.refresh_if_needed().await?;
        Ok(self.groups.iter().find(|e| e.name == name).map(|e| e.to_owned()))
    ...
    pub async fn get_by_gid(&mut self, gid: u32) -> Result<Option<GroupEntry>, std::io::Error> {
        self.refresh_if_needed().await?;
        Ok(self.groups.iter().find(|e| e.gid == gid).map(|e| e.to_owned()))
    ...
    pub async fn get_name_by_gid(&mut self, gid: u32) -> Result<Option<String>, std::io::Error> {
        self.refresh_if_needed().await?;
        Ok(self.groups.iter().find(|e| e.gid == gid).map(|e| e.name.to_owned()))
    }
